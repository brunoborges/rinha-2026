...
final class KdTreeTuning {
    ...
    static final boolean REFINE_PRESEED;

    static {
        String v = System.getenv("KDTREE_REFINE_BOUNDARY");
        REFINE_BOUNDARY = "1".equals(v) || "true".equalsIgnoreCase(v);
        v = System.getenv("KDTREE_REFINE_PRESEED");
        REFINE_PRESEED = "1".equals(v) || "true".equalsIgnoreCase(v);
    }
    ...
    static final int[] BUCKET_SCALE_Q16_TABLE;
    ...
    static {
        ...
        if (RELAX_INITIALLY_ENABLED) {
            ...
            // Precompute bucket table (16 visits per bucket)
            int maxBuckets = (RELAX_SOFT_CAP_INIT + RELAX_RANGE_INIT + 15) >>> 4;
            int[] bTbl = new int[maxBuckets + 1];
            for (int b = 0; b <= maxBuckets; b++) {
                bTbl[b] = relaxScaleQ16(b << 4);
            }
            BUCKET_SCALE_Q16_TABLE = bTbl;
        } else {
            RELAX_EPSILON_Q32_PER_VISIT = 0L;
            RELAX_MAX_SUB_Q16 = 0;
            RELAX_SCALE_Q16_TABLE = new int[0];
            BUCKET_SCALE_Q16_TABLE = new int[0];
        }
    }
    ...
    static int relaxScaleBucketQ16(int bucket) {
        if (!RELAX_INITIALLY_ENABLED) return 65536;
        if (bucket < BUCKET_SCALE_Q16_TABLE.length) {
            return BUCKET_SCALE_Q16_TABLE[bucket];
        }
        return 65536 - RELAX_MAX_SUB_Q16;
    }

    // ── MMAP ADVICE ───────────────────────────────────────────────────────────────────────────────
    static final boolean MMAP_HUGEPAGE_ENABLED;
    static final boolean MMAP_RANDOM_ENABLED;
    static final String MMAP_ADVICE_LABEL;

    static {
        String advice = System.getenv("KDTREE_MMAP_ADVICE");
        if (advice == null || advice.isEmpty()) {
            advice = "none";
        }
        advice = advice.toLowerCase();
        switch (advice) {
            case "hugepage" -> {
                MMAP_HUGEPAGE_ENABLED = true;
                MMAP_RANDOM_ENABLED = false;
                MMAP_ADVICE_LABEL = "hugepage (MADV_HUGEPAGE only)";
            }
            case "random" -> {
                MMAP_HUGEPAGE_ENABLED = false;
                MMAP_RANDOM_ENABLED = true;
                MMAP_ADVICE_LABEL = "random (MADV_RANDOM only)";
            }
            case "none" -> {
                MMAP_HUGEPAGE_ENABLED = false;
                MMAP_RANDOM_ENABLED = false;
                MMAP_ADVICE_LABEL = "none (no explicit advice)";
            }
            default -> { // "both"
                MMAP_HUGEPAGE_ENABLED = true;
                MMAP_RANDOM_ENABLED = true;
                MMAP_ADVICE_LABEL = "both (MADV_HUGEPAGE + MADV_RANDOM)";
            }
        }
    }
